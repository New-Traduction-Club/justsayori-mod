...
init -3 python:
    ...
    class Chat(object):
        ...
        def __init__(
            ...
            self._m1_handler__persistent_db = persistent_db
        ...
        def _m1_handler__load(self):
            """
            Load the chat stuff
            """
            
            for persist_key, value in self._m1_handler__persistent_db[self.label].items():
                if clm[persist_key]:
                    self.__dict__[persist_key] = value
        
        def _m1_handler__save(self):
            """
            Save dialogue to persistent
            """
            for persist_key, value in self.as_dict().items():
                if clm[persist_key]:
                    self._m1_handler__persistent_db[self.label][persist_key] = value
