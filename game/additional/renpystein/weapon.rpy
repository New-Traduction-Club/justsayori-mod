init python:
    ...
    class Weapon(object):
        def __init__(self, weaponName="fist", frameCount = 5, zoom_factor = 11, damage=25, projectile_type=None, cooldown=0.5, ads_idle=None, ads_fire=None):
            self.images = []
            self.playing = False
            self.frame = 0
            self.oldst = None
        ...
        def render_to(self, r, width, height, st, at, is_ads=False):
            ...
            if is_ads and self.ads_idle:
                # ADS Rendering Logic
                if self.playing and self.ads_fire:
