...
#             # --- Advanced Z-Axis Physics ---
#             self.z = 0.0          # Current Eye Height relative to ground (0.0 is base floor)
#             self.velocity_z = 0.0 # Vertical Velocity
            
#             # Physics Constants
#             self.GRAVITY = 35.0   # Strong gravity for snappy jumps
#             self.JUMP_FORCE = 8.5 # Initial upward impulse
#             self.CROUCH_DEPTH = -0.4 # How low we dip before jumping
            
#             # States
#             self.is_grounded = True
#             self.is_crouching = False # Pre-jump anticipation
#             self.crouch_timer = 0.0
#             self.crouch_duration = 0.06 # Very fast anticipation (60ms)
            
#         def get_ground_height_at(self, x, y):
#             """
#             Returns the floor height at specific coordinates.
#             Currently always 0.0, but this is where you will add logic for 
#             stairs or platforms later (e.g., reading a height map).
#             """
#             # Example logic for future:
#             # return self.wm.heightMap[int(x)][int(y)]
#             return 0.0

#         def trigger_jump(self):
#             """ Input handler: Initiates the jump sequence. """
#             if self.is_grounded and not self.is_crouching:
#                 self.is_crouching = True
#                 self.crouch_timer = self.crouch_duration

#         def update_physics(self, dt):
#             """ Handles gravity, velocity integration, and ground collision. """
...
#             bob_offset = 0.0
#             if is_moving and not effective_aiming and self.player.is_grounded:
#                 bob_speed = 10.0
#                 bob_amp = 1.0
#                 if is_running_state:
#                     bob_speed = 15.0
...
#             # --- 4a. WALL CASTING (MULTI-PASS) ---
#             zBuffer = [0.0] * i_width
            
#             # This variable tracks the lowest pixel we have drawn to so far for each column
#             # Since we render walls front-to-back, and walls are anchored to the floor,
#             # we start with the whole screen available (occlusion_limit = internal_height)
#             # and as we draw walls, this limit moves up
#             # If the limit reaches 0 (top of screen), we stop casting for this column
...
#                 occlusion_limit = self.internal_height
#                 first_hit_dist = 0.0
...
#                         if mapX < 0 or mapX >= self.mapWidth or mapY < 0 or mapY >= self.mapHeight:
#                             hit = -1
#                             break
...
#                     if first_hit_dist == 0.0:
#                         first_hit_dist = perpWallDist
...
#                     if lineHeight > 0:
#                         z_perspective_offset = (self.player.z * self.internal_height) / perpWallDist
#                         drawStart = -lineHeight / 2 + self.internal_height / 2 + horizon_offset + z_perspective_offset
#                         drawEnd = drawStart + lineHeight

#                         if side == 0:
#                             wall_depth = abs(1.0 / rayDirX)
#                         else:
#                             wall_depth = abs(1.0 / rayDirY)
                            
#                         back_dist = perpWallDist + wall_depth
                        
#                         back_line_height = int((self.internal_height / back_dist) * vertical_scale)
#                         back_z_offset = (self.player.z * self.internal_height) / back_dist
                        
#                         back_draw_start = -back_line_height / 2 + self.internal_height / 2 + horizon_offset + back_z_offset
...
#                         real_draw_end = min(drawEnd, occlusion_limit)
#                         real_draw_start = drawStart
                        
#                         if real_draw_end > real_draw_start:
...
#                             blit_y_on_screen = max(0, int(real_draw_start))
#                             blit_h = int(real_draw_end - blit_y_on_screen)
                            
#                             if blit_h > 0:
#                                 src_y_offset = blit_y_on_screen - int(drawStart)
                                
#                                 if src_y_offset < 0: src_y_offset = 0
#                                 if src_y_offset + blit_h > lineHeight: blit_h = lineHeight - src_y_offset
                                
#                                 canvas.blit(scaled_surf, (x, blit_y_on_screen), area=(0, src_y_offset, 1, blit_h))
                                
#                         roof_height = int(drawStart - back_draw_start)
#                         if roof_height > 0:
#                             texNum = world_map[mapX][mapY] - 1
                            
#                             roof_tex_index = texNum + 8
#                             if roof_tex_index < len(self.image_renders):
#                                 source_surf = self.image_renders[roof_tex_index]
#                             else:
#                                 source_surf = self.image_renders[texNum] # Fallback
                            
#                             slice_area = (texX, 0, 1, texHeight)
                            
#                             roof_scale_surf = pygame.transform.scale(source_surf.subsurface(slice_area), (1, roof_height))
                            
#                             roof_y = int(back_draw_start)
                            
#                             if roof_y < occlusion_limit:
#                                 available_h = occlusion_limit - roof_y
...
#                                     y_offset = -draw_y
#                                     draw_y = 0
#                                     available_h -= y_offset
                                
#                                 final_h = min(roof_height - y_offset, available_h)
                                
#                                 if final_h > 0:
#                                     canvas.blit(roof_scale_surf, (x, draw_y), area=(0, y_offset, 1, final_h))
                        
#                         top_of_block = back_draw_start if roof_height > 0 else drawStart
                        
#                         if top_of_block < occlusion_limit:
#                             occlusion_limit = top_of_block
                        
#                         if occlusion_limit <= 0:
#                             break # Screen column is full
                
#                 if zBuffer[x] == 0: zBuffer[x] = 100.0
...
#                 # Apply horizon offset + Z perspective offset for sprites
#                 z_sprite_offset = (self.player.z * self.internal_height) / transformY
                
#                 pitch_shift = sprite[3] if len(sprite) > 3 else 0
#                 f_drawStartY = self.internal_height / 2.0 - f_spriteHeight / 2.0 + horizon_offset + z_sprite_offset - pitch_shift
...
#                     # --- Buttons (Polling) ---
#                     # Shoot (Hold allowed): Button 5 (RB/R1) or Right Trigger (Axis 5)
#                     shoot_pressed = False
