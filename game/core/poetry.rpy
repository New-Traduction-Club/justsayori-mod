...
init 5 python:
    ...
    chatReg(
        Chat(
            ...
            label="poem_redux",
            unlocked=True,
            prompt=__("Let's talk about a poem"),
            random=False,
            category=[__("Poetry")]
        ),
...
label poem_redux:
    ...
    if madechoice == poem_bottles:
        call s_poem_bottles from _call_s_poem_bottles
        return

    elif madechoice == poem_sunshine:
        call s_poem_sunshine from _call_s_poem_sunshine
        return

    elif madechoice == poem_flower:
        call s_poem_flower from _call_s_poem_flower
        return
    elif madechoice == poem_last:
        call s_poem_last from _call_s_poem_last
        return

    elif madechoice == poem_fruits:
        call s_poem_fruits from _call_s_poem_fruits
        return

    elif madechoice == poem_angel:
        call s_poem_angel from _call_s_poem_angel
        return

    elif madechoice == poem_leaf:
        call s_poem_leaf from _call_s_poem_leaf
        return

    elif madechoice == poem_prose:
        call s_poem_prose from _call_s_poem_prose
        return

    elif madechoice == poem_afterlight:
        call s_poem_afterlight from _call_s_poem_afterlight
    return
